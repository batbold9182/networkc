...
import { FundingScreenStyle } from "../../styles/Styles";
    ...
    >
      <View style={FundingScreenStyle.container}>
        ...
        <View style={FundingScreenStyle.section}>
          ...
          />
          <Text
            style={FundingScreenStyle.button}
            onPress={async () => {
              const amt = parseFloat(withdrawAmount);
              if (!amt || amt <= 0) return alert("Please enter a valid amount");
              try {
                const res = await axios.post(
                  `${BACKEND_URL}/api/user/withdraw`,
                  { amount: amt },
                  {
                ...
                setWithdrawAmount("");
                alert(
                  `Withdrawn ${amt} PLN. New balance: ${res.data.balance} PLN`,
                );
              } catch (err: any) {
